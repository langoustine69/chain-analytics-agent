...
var errorResponse = (code, status, details) => {
  return jsonResponse(
    {
      error: {
        code,
...
function parseInput(entrypoint, value) {
...
function parseOutput(entrypoint, value) {
...
// src/invoke.ts
async function invokeHandler(entrypoint, rawInput, context) {
...
async function invoke(req, entrypointKey, runtime) {
...
var renderLandingPage = ({
  ...
  return html`<!DOCTYPE html>
    ...
    const streamPrice = streaming ? resolvePrice?.(entrypoint, "stream") ?? null : void 0;
    ...
    const inputSchema = entrypoint.input ? z.toJSONSchema(entrypoint.input) : void 0;
    const outputSchema = entrypoint.output ? z.toJSONSchema(entrypoint.output) : void 0;
...
var toDataLines = (value) => {
  return value.split(/\r?\n/).map((line) => line || "");
};
var buildSSEChunk = ({ event, data, id }) => {
  const lines = [];
  if (id) {
    lines.push(`id: ${id}`);
  }
...
var writeSSE = (controller, options) => {
  controller.enqueue(encoder.encode(`${buildSSEChunk(options)}
`));
};
var createSSEStream = (runner) => {
...
// src/stream.ts
async function stream(req, entrypointKey, runtime) {
  const entrypoint = runtime.agent.getEntrypoint(entrypointKey);
  if (!entrypoint) {
    return errorResponse("entrypoint_not_found", 404);
  }
  ...
  return createSSEStream(async ({ write, close }) => {
    const sendEnvelope = (payload) => {
      const currentSequence = payload.sequence != null ? payload.sequence : allocateSequence();
    ...
    try {
      ...
      const result = await entrypoint.stream(runContext, emit);
      sendEnvelope({
        kind: "run-end",
        runId,
  ...
  return {
    ...
    onBuild(runtime) {
      ...
      const actualHandlers = {
        ...
        landing: landingEnabled ? async (req) => {
          const origin = normalizeOrigin(req);
          const entrypoints = runtime.entrypoints.snapshot();
          const html2 = await renderLandingPage({
            meta,
            origin,
            entrypoints,
            activePayments,
          ...
          return new Response(String(html2), {
            headers: {
        ...
        tasks: async (req) => {
          ...
          try {
            ...
            if (!body || typeof body !== "object" || !("message" in body) || !("skillId" in body)) {
              return jsonResponse(
                {
                  error: {
          ...
          let rawInput;
          if (message.content && typeof message.content === "object" && !Array.isArray(message.content) && "text" in message.content) {
            try {
              rawInput = JSON.parse(
          ...
          } else if (message.content && typeof message.content === "object" && !Array.isArray(message.content) && "parts" in message.content && Array.isArray(message.content.parts) && message.content.parts.length > 0) {
            const firstPart = message.content.parts[0];
            if (firstPart && typeof firstPart === "object" && !Array.isArray(firstPart) && "text" in firstPart) {
              rawInput = firstPart.text;
            } else {
              rawInput = firstPart;
            }
          } else {
        ...
        listTasks: async (req) => {
          const url = new URL(req.url);
          const contextId = url.searchParams.get("contextId") || void 0;
          const statusParam = url.searchParams.get("status");
          const status = statusParam ? statusParam.includes(",") ? statusParam.split(",") : statusParam : void 0;
          ...
          if (status) {
            ...
            filteredTasks = filteredTasks.filter(
              (t) => statusArray.includes(t.status)
            );
          }
...
export { createSSEStream, http, invokeHandler, writeSSE };
